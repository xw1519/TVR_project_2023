...
% A_OptimalFreq(ntrials,tWindowInterval,Amps,isPlot,Chan,path,freqOpt) 
...
% FreqOptimal       - Optimal stimulation frequency - 140Hz
% isPlot            - Plot the different trials
% Chan              - EMG channel to analyse
% path              - Path

    
function T=deltaEMG_2_2(ntrials,tWindowInterval,Amps,isPlot,Chan,path)
...
% Default values
    ...
    T=table(Period,Amplitude,deltaEMG,SubjectID,EMGChan,gradient);
    
    % Plot results
    CatAmp = categorical(Amplitude,[0, 85, 170, 255]);
    if isPlot
        f{length(Amps)+1} = figure(length(Amps)+1);
        boxchart(CatAmp,T.gradient);
        ylabel('norm. ∆EMGrms [%]')
        ylim([0,10]);
        grid
        set(gca,'FontSize',24)
